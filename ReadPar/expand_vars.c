...
/* two function from getarg.c */
int iargc();
int getarg (int iarg, char *arg);
int lcase (char *string); /* from lcase.c */
...
int RP_expand_variables (char *string)
{
  ...
  for (i=0;i<L; ) {
	...
	ss = strchr(nametmp, ' ');
	if( s && ( ss == NULL || ss > s ) ){
	  /* this is needed to correctly treat cases like
	     ${ASKUSER[output image (img=)]} */
	  strcpy(eqval, s+1);
	  *s = '\0';
	}
	else{
