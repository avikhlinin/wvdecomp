...
int RP_Parse_Cont_String (char *line, char *value, char *comment, int *valuecont);
int  RP_Push_New_PF_Par (ino_t inode, char *name, char *value);
int RP_Parse_Par_String (char *line, char *name, char *value, char *comment,
			 int  *valuecont);
int RP_Parse_Group_String (char *line, char *name, char *comment);
int RP_RecordType (char *line);
int RP_PAR_Continued();
int RP_IsPAR_Continued (int yes);
int RP_IsInGroup (int yes);
...
{
  int i;
  for (i=RP_nfiles_loaded;i>=0;i--) {
    RP_Rem_PF_Pars (RP_sbuff_read[i].st_ino);
  }
  return 1;
}
